...
class Dynamic extends Component {
  ...
  handleForegroundTouchstart = () => {
    console.log('handleForegroundTouchstart');
  ...
  handleCoverLoaded = (detail: object, item?: any) => {
    console.log('tiezhi', detail, item);
  }
  ...
  canvasDrawElement = async (context, ratio) => {
    const {foreground, coverList = []} = this.state
    // 收集所有元素进行排序
  ...
  // 计算人物尺寸
  calcForegroundSize = () => {
    const {foreground, frame} = this.state
    const {originWidth, originHeight} = foreground
    const sceneInfo = this.getSceneInfoById('1', this.themeData.sceneList, 'sceneId')
            ...
            <View className={`crop ${foreground.remoteUrl ? '' : 'hidden'}`} id="crop">
              ...
              {coverList.map(item => {
                return <Sticker
                  ...
                  onImageLoaded={this.handleCoverLoaded.bind(this,item)}
